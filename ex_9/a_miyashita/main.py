...
import ex_5.a_miyashita.mfcc as mfcc
...
def my_MLP(input_shape, output_dim):
    ...
    model.add(Conv2D(16, (1,3), activation='relu', input_shape=input_shape))
...
def feature_extraction(data):
    ...
    lengths = np.array([y.size for y in data])
    pad_size = 50500 - lengths
    data = np.array([np.pad(data[i], (0,pad_size[i])) for i in range(pad_size.size)])
    """
    
    features = np.array([librosa.feature.mfcc(y=data[i,:12800], sr=8000) for i in range(data.shape[0])])
    return features
...
def main():
    ...
    """
    npz = np.load('bin/data_f2.npz')
    X_train = npz['train'][:,1:13,:,np.newaxis]
    X_test = npz['test'][:,1:13,:,np.newaxis]
    
    print(X_train.shape)
    print(X_test.shape)
    # np.savez('bin/data_f2', train=X_train, test=X_test)
    # sys.exit()
